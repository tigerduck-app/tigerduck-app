...
///
/// The three names are created on **any** server, not only the school's. Role resolution matches
/// folders by name and never by an RFC 6154 SPECIAL-USE attribute: Mail2000 sends none, and where
/// one is sent it is not reliably unique — a long-lived mailbox accumulates `Sent`, `Sent Items`
/// and `Sent Messages` left behind by the clients that have touched it, so choosing one of them to
/// file into is a guess. TigerDuck owns one unambiguous set instead, spelled the same everywhere,
/// and uses only that. The cost is accepted deliberately: mail TigerDuck files lands in TigerDuck's
/// Sent, and another client's Sent will not show it.
///
