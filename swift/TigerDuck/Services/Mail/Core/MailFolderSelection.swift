...
nonisolated enum MailFolderSelection: Hashable, Sendable {
    ...
    /// All mail: `mergedRoles` merged client-side, newest first. Never a folder name.
    case allMail

    /// The only folders All mail merges — the two that hold real correspondence. Drafts, junk,
    ...
    /// The one real folder this selection names, or nil for All mail — for the callers that
    ...
    /// All mail becomes the folders it merges, and a selection that resolves to nothing
...
///
/// A UID is unique only within its own folder, so Inbox and Sent routinely both hold a
