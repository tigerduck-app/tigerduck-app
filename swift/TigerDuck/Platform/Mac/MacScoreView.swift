...
struct MacScoreView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .automatic) {
                Picker(String(localized: "mac_score_ranking_scope_label"), selection: $viewModel.rankingScope) {
                    ForEach(ScoreViewModel.RankingScope.allCases) { scope in
                        Text(scope.displayName).tag(scope)
                    }
                }
                .pickerStyle(.segmented)
                .help(String(localized: "mac_score_ranking_scope_help"))
            }
            ToolbarItem(placement: .primaryAction) {
                ...
                .help(String(localized: "mac_score_refresh_help"))
            }
        }
        .task {
    ...
    private var header: some View {
        HStack(alignment: .firstTextBaseline, spacing: 24) {
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "mac_score_title"))
                    .font(.title2.bold())
                if let cached = viewModel.cachedAt {
                    Text(String(format: String(localized: "mac_score_cached_value"), cached.formatted(.relative(presentation: .named))))
                        .font(.caption)
    ...
    private func rankSummary(_ ranking: SemesterRanking) -> some View {
        ...
        return HStack(spacing: 22) {
            statColumn(label: String(localized: "mac_score_gpa_label"), value: stats.gpa.map { String(format: "%.2f", $0) } ?? "—")
    ...
    private var emptyState: some View {
        VStack(spacing: 6) {
            Image(systemName: "chart.bar")
                .font(.title)
                .foregroundStyle(.secondary)
            Text(String(localized: "mac_score_empty_title"))
                .font(.headline)
            Text(String(localized: "mac_score_empty_hint"))
                .font(.callout)
                .foregroundStyle(.secondary)
    ...
    private var tableHeader: some View {
        HStack(spacing: 12) {
            Text(String(localized: "mac_score_course_column"))
                .frame(maxWidth: .infinity, alignment: .leading)
            Text(String(localized: "course_detail_credits_label"))
                .frame(width: 64, alignment: .trailing)
            Text(String(localized: "mac_score_grade_column"))
    ...
    private func displayTerm(_ raw: String) -> String {
        ...
        return String(format: String(localized: "mac_score_term_value"), year, sem)
    }
}
#endif
