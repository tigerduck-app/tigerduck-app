...
2. `MailCharsetResolver` — an overridable charset-label → `String.Encoding` hook for
   RFC 2047 header decoding, so the app's Appendix A.5 rules (Big5-as-HKSCS, GBK-as-
   GB18030, …) apply to sender names and subjects (`Core/MailCharsetResolver.swift`,
   `Extensions/String+QuotedPrintable+MIMEHeader.swift`).
